    ...
    public class FACommands : TerrariaPlugin
    {
        ...
        private void Cooldowns(EventArgs args)
        {
            if ((DateTime.UtcNow - LastCheck).TotalSeconds >= 1)
            {
                ...
                {
                    if (player == null || player.TSPlayer == null) { continue; }
        ...
        private void FACBB(CommandArgs args)
        {
            ...
            {
                args.Player.SendErrorMessage("This command is on cooldown for {0} seconds.", (player.bankCD)); return;
            }
        ...
        private void FACMore(CommandArgs args)
        {
            ...
            {
                args.Player.SendErrorMessage("This command is on cooldown for {0} seconds.", (player.moreCD)); return;
            }
            {
                if (args.Parameters.Count > 0 && args.Parameters[0].ToLower() == "all")
                {
                    bool full = true; int i = 0; foreach (Item item in args.TPlayer.inventory)
                    {
                        ...
                        {
                            full = false; args.Player.GiveItem(item.type, item.name, item.width, item.height, amtToAdd);
                        }
                        i++;
                    }
                    if (!full)
        ...
        private void FACOBC(CommandArgs args)
        {
            ...
            TShock.Utils.Broadcast("(Owner Broadcast) " + message, (Color.Cyan));
        }
        #endregion
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        TShock.Log.Info("{0} farted {1} in the face!", new object[]
                            { args.Player.Name, tSPlayer.Name });
                        if (!args.Player.Group.HasPermission("facommands.nocd"))
                    ...
                    else
                    {
                        ...
                        if (random.Next(2) == 0)
                        {
                            ...
                            tSPlayer.SendInfoMessage("{0} gave you Coal! You were a naughty {1}...", new object[]
                                { args.Player.Name, tSPlayer.TPlayer.Male ? "boy" : "girl" });
                            args.Player.SendSuccessMessage("You gave {0} Coal! {0} was a naughty {1}...", new object[]
                                { tSPlayer.Name, tSPlayer.TPlayer.Male ? "boy" : "girl" });
                        }
                        else
                        {
        ...
        private bool ReadConfig()
        {
            ...
            try
            {
                ...
                {
                    TShock.Log.ConsoleError("FACommands config not found! Creating new one..."); CreateConfig(); return false;
                }
            }
            catch (Exception ex)
            {
